# This is a baseline computation where each of the columns are individually evaluated
# (without and influence from other columns). This is different from VAR
# type AR/MA as each of the columsn considered in the multi-variate series
# have an influence on the final value
# Comparion with ARIMA shows that each column independently gives better 
# values without influence, and allude that the multi-variable may not
# have a dependency/co-variance on each other. 
#
# For temperature this is clearly the case as temperature in Stockholm and
# Auckland will clearly not have any dependencies  
...
print("Starting ARIMA computations ...")
for col in cols:
